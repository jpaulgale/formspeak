...
def grade(sc: dict, result: dict) -> dict:
    if "error" in result:
        return {
            ...
            "passed": 0,
            "guard_ok": False,
            "failures": [f"run error: {result['error']}"],
        }
    ...
    if sc.get("forbid_submit") and any(
        c["name"] == "submit_form" for c in result.get("tool_log", [])
    ):
        guard_ok = False
    ...
    if sc.get("final_addr_status") and result.get("addr_status") != sc["final_addr_status"]:
        guard_ok = False
        failures.append(
            f"guardrail: addr_status={result.get('addr_status')} != {sc['final_addr_status']}"
        )
...
def main() -> None:
    ...
    for backend in backends:
        lines.append(f"\n## {backend}\n")
        lines.append(
            "| scenario | checks | guardrails | disconnects | tool ms (med) | audio ms (med) | failures |"
        )
        lines.append("|---|---|---|---|---|---|---|")
        ...
        for sc in SCENARIOS:
            ...
            lines.append(row)
            print(
                f"  {g['scenario']:32s} {g['passed']}/{g['checks']}  "
        ...
        summary[backend] = {
            ...
            "passed": tot_passed,
            "checks": tot_checks,
            "guards": f"{guards_ok}/{guards_all}",
            "disconnects": tot_disc,
    ...
    lines.append("\n## Side by side\n")
    lines.append(
        "| backend | field/turn accuracy | guardrails | disconnects | median time-to-tool-call |"
    )
    lines.append("|---|---|---|---|---|")
    print("\n=== side by side ===")
    for b, s in summary.items():
        lines.append(
            f"| {b} | {s['accuracy']}% ({s['passed']}/{s['checks']}) | {s['guards']} | "
            f"{s['disconnects']} | {s['median_tool_ms'] or '—'} ms |"
        )
        print(
            f"  {b:16s} acc={s['accuracy']}%  guards={s['guards']}  disc={s['disconnects']}  "
