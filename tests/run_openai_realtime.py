...
async def run_scenario(
    http: aiohttp.ClientSession, key: str, model: str, sc: dict, reasoning: str | None = None
) -> dict:
    ...
    async with http.ws_connect(
        url, headers={"Authorization": f"Bearer {key}"}, max_msg_size=16 * 1024 * 1024
    ) as ws:
...
async def main() -> None:
    ...
    results_dir = (
        HERE / "results" / (args.model + (f"-r{args.reasoning}" if args.reasoning else ""))
    )
